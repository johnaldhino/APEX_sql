...
def APEX_get_end(runnum):
    ...
    if not DEBUG:
        try:
            # db = pymysql.connect(host='halladb.jlab.org', user='apexrw',a passwd='Rw4p3xdb', db="apexdb")
           db = pymysql.connect(host='npls3.physics.gla.ac.uk', user='apexuser', db='apexdb' )
            # db = pymysql.connect(host='nplsdsf3.physics.gla.ac.uk', user='apexuser' )
    ...
    try:
        comment_file = open("/home/johnw/Apex_runlist/logfiles/halog_end_" + str(runnum) +".epics","r")
...
#event_count
    ...
    insert_query = "REPLACE INTO " + EXP + "runlist (run_number, event_count)"
    ...
    record_to_insert = [runnum, str(start_comment),str(No_Events)]
...
#        return(insert_query,record_to_insert,HV_insert_query,HV_result,found_com)
        return(insert_query,record_to_insert,found_com)
...
def APEX_mysql(query_vars,query_values, HV_vars, HV_vals, runnum):
    ...
    if not DEBUG:
        try:
            db = pymysql.connect(host='halladb.jlab.org', user='apexrw', passwd='Rw4p3xdb', db="apexdb")
        except pymysql.Error:
            print("Could not connect to database. Please ensure that the paper runlist is kept up-to-date. Please email  ( jwilli@jlab.org) and include what run number this message appeared on.")
            sys.exit(1)
