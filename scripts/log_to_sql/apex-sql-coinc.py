...
try:
    ...
    print 'Run Number not found by the MySQL script'
...
if not DEBUG:
    try:
        ...
        print 'Could not connect to database. Please ensure that the paper runlist is kept up-to-date. Please email John Williamson  (jwilli@jlab.org) and include what run number this message appeared on.'
        sys.exit(1)
...
if not DEBUG:
    ...
    if nEvtAll>0:
        print 'This run number is already in existence in the run_list. Please email John Williamson and include what run number this message appeared on.'
        sys.exit(1)
...
try:
    ...
    print 'Title file seems to be missing. Please email John Williamson and include what run number this message appeared on.'

#Get variables from epics
# target_encoder = float(caget('APEXPOS'))
target_encoder = 0
...
momentumR = caget('HacR_D1_P0rb')
momentumL = caget('HacL_D1_P0rb')
angleR = caget('HacR_CalcAngle')
angleL = caget('HacL_CalcAngle')
...
#in hindsight, this next part would do better as a function.
    ...
    print 'Prescale file seems to be missing. Please email John Williamson and include what run number this message appeared on.'
...
insert_query += "momentumL, "
insert_query += "angleL, "
insert_query += "momentumR, "
insert_query += "angleR, "
